  ...
  return {
      ...
      if (type === 'inc') {
        ...
        listIncomes.forEach(listIncome => {
          ...
          if (incID === `${type}-${item.id}`) {
            document.querySelector(`#${incID}`).innerHTML = `<strong>${
              item.description
            }</strong>
                        <span class="inc-value">${formatNumber(
                          item.value,
                          type
                        )}</span>
                        <a href="#" class="btn">
        ...
        listExpenses.forEach(listExp => {
          ...
          if (expID === `${type}-${item.id}`) {
            document.querySelector(`#${expID}`).innerHTML = `<strong>${
              item.description
            }</strong>
                        <span class="exp-value">${formatNumber(
                          item.value,
                          type
                        )}</span>
                        <em class="exp-percent">${item.percentage}</em>
    ...
    addItemToForm: type => {
      ;(document.querySelector(UISelectors.inputType).value = type), // Will be either inc or exp
        ...
        ).value = BudgetCtrl.getCurrentItem(type).value)
...
const App = ((BudgetCtrl, UICtrl) => {
  ...
  const updateItemSubmit = e => {
    ...
    if (input.description !== '' && input.value > 0 && !isNaN(input.value)) {
      // update Item
      const updatedItem = BudgetCtrl.updateItem(
        input.type,
        input.description,
        input.value
      )
