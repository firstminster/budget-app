...
var UICtrl = (function () {
  ...
  return {
    ...
    displayBudget: function displayBudget(item) {
      ...
      document.querySelector(
        UISelectors.budgetLabel
      ).textContent = formatNumber(item.budget, type)
      document.querySelector(
        UISelectors.incomeLabel
      ).textContent = formatNumber(item.totalInc, 'inc')
      document.querySelector(
        UISelectors.expensesLabel
      ).textContent = formatNumber(item.totalExp, 'exp')
    ...
    addItemToForm: function addItemToForm(type) {
      ;(document.querySelector(UISelectors.inputType).value = type), // Will be either inc or exp
        (document.querySelector(
          UISelectors.inputDesc
        ).value = BudgetCtrl.getCurrentItem(type).description),
        (document.querySelector(
          UISelectors.inputVal
        ).value = BudgetCtrl.getCurrentItem(type).value) // Show update state
    ...
    displayMonth: function displayMonth() {
      ...
      document.querySelector(UISelectors.dateLabel).textContent =
        daysArr[day] + ' ' + monthsArr[month] + ' ' + year
    },
    changedType: function changedType() {
      var fields = document.querySelectorAll(
        UISelectors.inputType +
          ',' +
          UISelectors.inputDesc +
          ',' +
          UISelectors.inputVal
    ...
    clearInput: function clearInput() {
      ...
      ;(document.querySelector(UISelectors.inputType).value = type),
        ...
        (document.querySelector(UISelectors.inputVal).value = '') // Clear current Items
...
var App = (function (BudgetCtrl, UICtrl) {
  ...
  var loadEventListeners = function loadEventListeners() {
    ...
    document
      .querySelector(UISelectors.inputBtn)
    ...
    document
      ...
      .addEventListener('click', deleteItem) // Edit icon click event

    document
      .querySelector(UISelectors.itemList)
      .addEventListener('click', editItemClick) // Update Item submit event

    document
      .querySelector(UISelectors.updateBtn)
      .addEventListener('click', updateItemSubmit) // Change Input fields style
  ...
  var editItemClick = function editItemClick(e) {
    ...
    if (e.target.classList.contains('edit-btn')) {
      ...
      document
        .querySelector(UISelectors.inputType)
  ...
  var updateItemSubmit = function updateItemSubmit(e) {
    ...
    if (input.description !== '' && input.value > 0 && !isNaN(input.value)) {
      // update Item
      var updatedItem = BudgetCtrl.updateItem(
        input.type,
        input.description,
        input.value
      )
